-- Keep reviewed content imports and dispute state changes atomic. These
-- functions are intentionally service-role only because they accept an
-- explicit actor/user id and operate on server-owned tables.
...
begin
  ...
  insert into technical_content_audit_events (
    ...
    (p_family #>> '{review,founderOverride}')::boolean,
    case when (p_family #>> '{review,founderOverride}')::boolean then now() + interval '30 days' else null end
  );
