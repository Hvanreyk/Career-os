...
export function Footer() {
  return (
    <footer className="border-t border-white/6 bg-navy-950/80 backdrop-blur-sm">
            ...
            <div className="flex items-center gap-3">
              {socials.map(({ label, href, svg }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-9 h-9 rounded-lg bg-white/5 border border-white/8 flex items-center justify-center text-slate-400 hover:text-gold-400 hover:border-gold-400/30 transition-all"
                >
                  {svg}
                </a>
              ))}
              ...
              <ul className="space-y-3">
                {items.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className="text-sm text-slate-400 hover:text-gold-300 transition-colors"
                    >
                      {item.label}
                    </Link>
                  </li>
